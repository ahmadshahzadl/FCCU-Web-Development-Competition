  ...
  return (
    <div className="card hover:shadow-md transition-shadow duration-300">
      ...
      <div className="flex items-start space-x-3 sm:space-x-4 mb-4 sm:mb-6 pb-3 sm:pb-4 border-b border-gray-200 dark:border-gray-700 transition-colors duration-300">
        <div className="p-2 sm:p-2.5 rounded-lg bg-green-100 dark:bg-green-900/30 transition-colors duration-300 flex-shrink-0">
          <Mail className="h-4 w-4 sm:h-5 sm:w-5 text-green-600 dark:text-green-400 transition-colors duration-300" />
        </div>
      ...
      {/* Info Banner */}
      <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 dark:border-blue-400 rounded-lg transition-colors duration-300">
      ...
      {/* Add Domain Form */}
      <form onSubmit={handleAddDomain} className="mb-4 sm:mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          <div className="flex-1 relative">
            <Mail className="absolute left-2.5 sm:left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 sm:h-5 sm:w-5 text-gray-400 dark:text-gray-500 transition-colors duration-300" />
            <input
              type="text"
              value={newDomain}
          ...
          <button
            type="submit"
            disabled={loading}
            className="btn btn-primary flex items-center space-x-2 disabled:opacity-50 w-full sm:w-auto sm:min-w-[140px] justify-center text-sm sm:text-base px-4 py-2.5 sm:py-2.5"
          >
      ...
      <div>
        ...
        {config.allowedEmailDomains.length === 0 ? (
          <div className="p-6 sm:p-8 text-center border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/30 transition-colors duration-300">
            <Mail className="h-10 w-10 sm:h-12 sm:w-12 text-gray-400 dark:text-gray-500 mx-auto mb-2 sm:mb-3 transition-colors duration-300" />
            <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400 mb-1 transition-colors duration-300">
          ...
          <div className="space-y-2">
            {config.allowedEmailDomains.map((domain, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 sm:p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 transition-all duration-300 group"
              >
                <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                  <div className="p-1.5 rounded bg-green-100 dark:bg-green-900/30 transition-colors duration-300 flex-shrink-0">
                    <CheckCircle2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-green-600 dark:text-green-400 transition-colors duration-300" />
                  </div>
                  <span className="text-xs sm:text-sm font-medium text-gray-900 dark:text-white transition-colors duration-300 truncate">
                    {domain}
                  </span>
                </div>
                <button
                  ...
                  disabled={loading}
                  className="btn btn-danger flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm px-2 sm:px-3 py-1.5 disabled:opacity-50 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200 flex-shrink-0 touch-manipulation"
                >
